    ...
    >
      <Modal.Body className="show-grid p-0 ">
        <Container className="border border-white rounded-3 border-5 py-3">
          <Button
            variant="link"
            className="position-absolute top-0 end-0 text-decoration-none text-dark"
            id="close-btn"
            onClick={onHide}
          >
            X
          ...
          <Row className="d-flex">
            ...
            <Col
              lg={5}
              className="d-flex flex-column justify-content-between ps-3 mt-3 mt-md-0"
            >
              <div>
                ...
                <ul className="list-inline small ps-0">
                  {project?.technologies.map((item) => (
                    <li
                      className="list-inline-item p-1 me-1 my-1 border"
                      key={item.id}
                    >
                      {item}
                    </li>
                ...
                <h6>ABOUT</h6>
                <div>
                  {project?.description.split("linebreak").map((line) => (
                    <p>{line}</p>
                  ))}
                </div>
              </div>
